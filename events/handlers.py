...
from events.kb import get_events_inline_keyboard, get_events_menu_keyboard
...
async def show_events(message: types.Message, state: FSMContext, events, user_id):
    for event in events:
        if user_id == event.owner.user_id:
            kb = await get_events_inline_keyboard(['connect', 'delete'], event.id)
        else:
            kb = await get_events_inline_keyboard(['connect'], event.id)
        await show_event(message, event, kb)
    async with state.proxy() as data:
        if data['offset'] < data['total']:
            kb = await get_events_inline_keyboard(['+1', '+5'])
...
async def create_event(message: types.Message):
    ...
    kb = await get_events_menu_keyboard(['cancel'])
...
async def create_event_finish(message: types.Message, state: FSMContext):
    ...
    await message.answer(local('phrases', 'finish_create_event'), reply_markup=kb, parse_mode="HTML")
    await state.finish()
...
@ag.dp.callback_query_handler(lambda c: c.data.startswith('btn_connect') or c.data.startswith('btn_delete'), state=['*'])
async def process_callback_btn_event(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if 'btn_connect' == btn:
        ...
        await chat_init(callback_query.message, callback_query.from_user.id, callback_query.from_user.id, callback_query.from_user.username, event)
    elif 'btn_delete' == btn:
        kb = await get_events_inline_keyboard(['yes', 'no'], index)
